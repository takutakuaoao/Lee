<?php

declare(strict_types=1);

namespace Lee\Live\Domain\Model\Live\LiveHouse;

use InvalidArgumentException;
use Lee\Live\Shared\ValueObject;

final class Prefecture implements ValueObject
{
    private const TRIM_PATTERN = '[都県府]';

    private static $prefectures = [
        '北海道', '青森', '岩手', '宮城', '秋田', '山形', '福島', '茨城', '栃木', '群馬',
        '埼玉', '千葉', '東京', '神奈川', '新潟', '富山', '石川', '福井', '山梨', '長野',
        '岐阜', '静岡', '愛知', '三重', '滋賀', '京都', '大阪', '兵庫', '奈良', '和歌山',
        '鳥取', '島根', '岡山', '広島', '山口', '徳島', '香川', '愛媛', '高知', '福岡',
        '佐賀', '長崎', '熊本', '大分', '宮崎', '鹿児島', '沖縄',
    ];

    /** @var string */
    private $value;

    public function __construct(string $prefecture)
    {
        $trimPattern = self::TRIM_PATTERN . '$';
        $prefecture  = preg_replace('/' . $trimPattern . '/u', '', trim($prefecture));

        if (!in_array($prefecture, self::$prefectures)) {
            throw new InvalidArgumentException('prefecture value is invalid.');
        }

        $this->value = $prefecture;
    }

    public function equal(ValueObject $value): bool
    {
        return get_class($this) === get_class($value) && $this->value === $value->value;
    }

    public function __toString(): string
    {
        return (string)$this->value;
    }
}
